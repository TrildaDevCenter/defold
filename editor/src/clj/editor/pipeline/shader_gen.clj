...
(ns editor.pipeline.shader-gen
  ...
  (:import [com.dynamo.bob.pipeline ShaderProgramBuilder ShaderProgramBuilderEditor Shaderc$ShaderResource]
...
(defn- make-attribute-reflection-info [^Shaderc$ShaderResource attribute]
  (let [shader-resource-type (.-type attribute)
    ...
    ;; map with attribute-info keys from the material to obtain a fully-formed
    ;; attribute-info map.
    {:name (.-name attribute)
...
(defn transpile-shader-source [^String shader-proj-path ^String shader-source ^long max-page-count]
  {:pre [(string? shader-proj-path)
